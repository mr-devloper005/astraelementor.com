...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[#ced9d1] bg-white shadow-[0_18px_54px_rgba(21,51,58,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_26px_70px_rgba(21,51,58,0.14)] ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-[var(--slot4-media-bg)] ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[rgba(21,51,58,0.76)] via-transparent to-transparent" />
      ...
      <div className="p-5 sm:p-6">
        ...
        {summary ? <p className="mt-4 line-clamp-3 text-sm font-semibold leading-7 text-[var(--slot4-soft-muted-text)]">{summary}</p> : null}
      ...
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[1440px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="overflow-hidden rounded-[2.4rem] border border-[#ced9d1] bg-white shadow-[0_24px_70px_rgba(21,51,58,0.09)]">
            <div className="grid gap-8 bg-[var(--slot4-blue-band)] px-6 py-8 text-white md:grid-cols-[0.8fr_1.2fr] lg:px-10">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.28em] text-white/72">{pagesContent.search.hero.badge}</p>
              ...
              <form action="/search" className="self-end rounded-[2rem] border border-white/16 bg-white/12 p-4 backdrop-blur sm:p-5">
                <input type="hidden" name="master" value="1" />
                <label className="flex items-center gap-3 rounded-2xl border border-white/14 bg-white px-4 py-3">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <label className="flex items-center gap-2 rounded-2xl border border-white/14 bg-white px-4 py-3">
                  ...
                  <select name="task" defaultValue={task} className="rounded-2xl border border-white/14 bg-white px-4 py-3 text-sm font-black text-[var(--slot4-page-text)] outline-none">
                    <option value="">All content types</option>
                    {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                  </select>
                </div>
                <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-[0.9rem] bg-[#f0bc2b] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--slot4-page-text)] transition hover:-translate-y-0.5" type="submit">Search</button>
              </form>
            </div>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/article" className="inline-flex items-center gap-2 rounded-full border border-[#ced9d1] bg-white px-5 py-3 text-sm font-black">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-[#ced9d1] bg-white/70 p-10 text-center">
              <p className="text-2xl font-black tracking-[-0.04em]">No matching posts found.</p>
              <p className="mt-3 text-sm font-semibold text-[var(--slot4-soft-muted-text)]">Try a different keyword, task type, or category.</p>
            </div>
          )}
        </section>
      </main>
